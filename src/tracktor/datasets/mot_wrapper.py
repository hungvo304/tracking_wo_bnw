        ...
        for s in sequences:
            if dets == '17':
                self._data.append(MOT17Sequence(
                    seq_name=s, dets='DPM17', **dataloader))
                self._data.append(MOT17Sequence(
                    seq_name=s, dets='FRCNN17', **dataloader))
                self._data.append(MOT17Sequence(
                    seq_name=s, dets='SDP17', **dataloader))
            else:
                self._data.append(MOT17Sequence(
                    seq_name=s, dets=dets, **dataloader))
...
class MOT17LOWFPSWrapper(MOT17Wrapper):
    ...
    def __init__(self, split, dataloader):
        ...
        self._data = []
        for s in sequences:
            self._data.append(MOT17LOWFPSSequence(
                split=split, seq_name=s, dets='FRCNN17', **dataloader))
